angular.module('tmg').controller 'SchemeCtrl', ['$scope', '$rootScope', '$cookies', '$state', ($scope, $rootScope, $cookies, $state)->
	...
	$scope.cab = (cab)->
		...
		$scope.map.img['m'+cab] = true

	if list.length > 0
		$scope.cab list[0].cab

	$scope.showFloor = (floor)->
		$scope.floor = floor
		if floor == 1
			$scope.cab $scope.menuoneleft[0].cab
		else
			$scope.cab $scope.menutwoleft[0].cab
